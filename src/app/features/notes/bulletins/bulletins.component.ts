// bulletins.component.ts — écran de génération des bulletins PDF
// Boutons : configurer | bulletins classe | PV | fiche saisie | par élève aperçu/dl
...
<div class="bl-host">
  ...
  <div class="bl-bar">
    ...
    <!-- Résumé config actuelle + bouton modifier -->
    ...
    <button class="bl-btn bl-btn--outline" (click)="ouvrirConfig()">
      <svg width="13" height="13" viewBox="0 0 16 16" fill="none">
        <path d="M8 2a6 6 0 100 12A6 6 0 008 2zM8 6v4M8 10.5v.5" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/>
      </svg>
      Configurer
    </button>

    <button class="bl-btn bl-btn--outline" [disabled]="!ctrlClasse.value || loading()" (click)="charger()">
      <svg width="13" height="13" viewBox="0 0 16 16" fill="none">
        <path d="M13.5 8A5.5 5.5 0 112.6 5M2 2v4h4" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
      </svg>
      Charger
    </button>
    ...
    @if (rows().length > 0) {
      <span class="bl-sep"></span>
      <!-- Bulletins classe -->
      <button class="bl-btn bl-btn--primary" [disabled]="genAll()" (click)="telechargerClasse()">
      ...
      <!-- PV -->
      <button class="bl-btn bl-btn--outline" [disabled]="genPV()" (click)="telechargerPV()">
        @if (genPV()) { <span class="bl-spinner" style="border-top-color:#185FA5"></span> } @else {
          <svg width="13" height="13" viewBox="0 0 16 16" fill="none"><rect x="2" y="2" width="12" height="12" rx="1.5" stroke="currentColor" stroke-width="1.3"/><path d="M5 6h6M5 9h4" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/></svg>
        }
        PV
      </button>
      <!-- Fiche saisie manuscrite -->
      <button class="bl-btn bl-btn--outline" (click)="telechargerFicheSaisie()">
        <svg width="13" height="13" viewBox="0 0 16 16" fill="none"><path d="M11 2l3 3-8 8H3v-3l8-8z" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"/></svg>
        Fiche saisie
      </button>
    }
  </div>
    ...
    <div class="bl-skeleton">
      @for (_ of [1,2,3,4,5]; track $index) {
        <div class="bl-sk-row">
          ...
          <div class="bl-sk-cell"></div><div class="bl-sk-cell"></div><div class="bl-sk-cell"></div>
        </div>
      }
    </div>
  }
  ...
  @if (!loading() && rows().length > 0) {
      ...
      <table class="bl-table">
        ...
        <tbody>
          @for (row of rows(); track row.eleve.id_eleve) {
            <tr class="bl-tr">
              <td class="bl-td bl-td--name">{{ row.eleve.nom }} {{ row.eleve.prenom }}</td>
              @for (moy of row.moySeqs; track $index) {
                <td class="bl-td bl-td--center" [class.bl-ok]="(moy??-1)>=10" [class.bl-bad]="moy!==null&&moy<10">
                  {{ moy !== null ? moy.toFixed(2) : '—' }}
                </td>
              }
              @if (config.sequences.length > 1) {
                <td class="bl-td bl-td--center bl-td--trim" [class.bl-ok]="(row.moyTrim??-1)>=10" [class.bl-bad]="row.moyTrim!==null&&row.moyTrim<10">
                  {{ row.moyTrim !== null ? row.moyTrim.toFixed(2) : '—' }}
                </td>
              }
              <td class="bl-td bl-td--center">{{ row.rang ?? '—' }}/{{ rows().length }}</td>
              <td class="bl-td bl-td--center">
                <span class="bl-mention" [class]="mentionCls(row.moyTrim ?? row.moySeqs[0])">
                  {{ mention(row.moyTrim ?? row.moySeqs[0]) }}
                </span>
              </td>
              <td class="bl-td bl-td--actions">
                <button class="bl-icon-btn" title="Aperçu" (click)="apercu(row.eleve)">
                  <svg width="13" height="13" viewBox="0 0 16 16" fill="none"><circle cx="8" cy="8" r="3" stroke="currentColor" stroke-width="1.3"/><path d="M1 8s2.5-5 7-5 7 5 7 5-2.5 5-7 5-7-5-7-5z" stroke="currentColor" stroke-width="1.3"/></svg>
                </button>
                <button class="bl-icon-btn" title="Télécharger" (click)="telecharger(row.eleve)">
                  <svg width="13" height="13" viewBox="0 0 16 16" fill="none"><path d="M8 2v8M5 7l3 3 3-3M3 13h10" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>
                </button>
              </td>
            </tr>
    ...
    <div class="bl-foot">
      <span class="bl-foot-info">
        {{ rows().length }} élève(s) · {{ config.sequences.length }} séquence(s) · {{ config.annee }}
      </span>
    </div>
  }
  ...
  styles: [`
    ...
    .bl-bar{display:flex;align-items:center;flex-wrap:wrap;gap:8px;padding-bottom:12px;border-bottom:0.5px solid rgba(0,0,0,.09)}
    .bl-select{height:32px;padding:0 10px;border:0.5px solid rgba(0,0,0,.18);border-radius:6px;font-size:13px;background:white;cursor:pointer}
    ...
    .bl-btn{height:32px;padding:0 14px;border-radius:6px;font-size:13px;cursor:pointer;display:inline-flex;align-items:center;gap:5px;transition:opacity .1s}
    .bl-btn:disabled{opacity:.35;cursor:default}
    .bl-btn--outline{background:white;color:#333;border:0.5px solid rgba(0,0,0,.18)}
    ...
    .bl-sk-cell{height:30px;min-width:68px;border-radius:4px;background:linear-gradient(90deg,#f0f0f0 25%,#e6e6e6 50%,#f0f0f0 75%);background-size:200% 100%;animation:shimmer 1.2s infinite}
    ...
    .bl-th{padding:7px 10px;font-weight:500;font-size:11px;background:#f8f8f8;color:#666;border-bottom:0.5px solid rgba(0,0,0,.08);text-align:center;white-space:nowrap}
    .bl-th--trim{background:#EBF3FC;color:#0C447C}
    .bl-td{padding:7px 10px;border-bottom:0.5px solid rgba(0,0,0,.05);vertical-align:middle}
    .bl-td--name{font-weight:500}
    .bl-td--center{text-align:center}
    .bl-td--trim{background:#EBF3FC;font-weight:600;color:#0C447C}
    .bl-td--actions{text-align:center;display:flex;gap:4px;justify-content:center}
    ...
    .bl-icon-btn{width:28px;height:28px;padding:0;border:0.5px solid rgba(0,0,0,.12);background:white;cursor:pointer;border-radius:5px;display:inline-flex;align-items:center;justify-content:center;color:#555;transition:background .1s}
    .bl-icon-btn:hover{background:#EBF3FC;color:#185FA5;border-color:#B5D4F4}
    .bl-spinner{width:13px;height:13px;border-radius:50%;border:2px solid rgba(0,0,0,.1);border-top-color:#fff;animation:spin .7s linear infinite;display:inline-block}
...
export class BulletinsComponent implements OnInit {
  ...
  rows = computed(() => {
    ...
    const withMoy = eleves.map(eleve => {
      const moySeqs = this.config.sequences.map(seq => this._moySeq(eleve, seq, matieres));
      const valides = moySeqs.filter((v): v is number => v !== null);
      const moyTrim = valides.length ? valides.reduce((a, b) => a + b, 0) / valides.length : null;
  ...
  telecharger(eleve: Eleve): void {
    ...
    this.pdfSvc.telecharger(blob, `bulletin_${eleve.nom}_${this.config.sequences.join('-')}.pdf`);
    this.snack.open('Bulletin téléchargé', '', { duration: 2000 });
  }

  async telechargerClasse(): Promise<void> {
    this.genAll.set(true); await Promise.resolve();
    const bulletins = this._eleves().map(e => this._buildBulletinData(e));
    const cls = (this.data.getClasses() ?? []).find(c => c.id_classe === this.ctrlClasse.value)?.nom_classe ?? '';
  ...
  async telechargerPV(): Promise<void> {
    this.genPV.set(true); await Promise.resolve();
    const matieres = this._matieres();
