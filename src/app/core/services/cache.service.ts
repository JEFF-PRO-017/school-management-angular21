...
export class CacheService {
  ...
  private _familles   = signal<CacheEntry<Famille[]>   | null>(null);
  private _classes    = signal<CacheEntry<Classe[]>    | null>(null);
  private _frais      = signal<CacheEntry<FraisConfig[]>| null>(null);
  private _enseignants= signal<CacheEntry<Enseignant[]>| null>(null);
  private _matieres   = signal<CacheEntry<MatiereConfig[]>| null>(null);

  // ── Groupe B : Élèves (cache session) ───────────
  private _eleves     = signal<CacheEntry<Eleve[]>     | null>(null);

  // ── Groupe D : Snapshots ─────────────────────────
  private _soldes     = signal<CacheEntry<SoldeSnap[]> | null>(null);
  private _bulletins  = signal<CacheEntry<BulletinSnap[]>| null>(null);
