...
export class WhatsappService {
  ...
  send_message_bulk(msgs: Message[]) {

  }

  /** Envoie un texte libre via Gupshup. Seule fonction qui appelle l'API. */
  private async send_message(msg: Message): Promise<{ ok: boolean; messageId?: string }> {
    ...
    try {
      ...
      return { ok: reponse.status === 'submitted', messageId: reponse.messageId };
    } catch (err) {
      console.error('[WhatsappService] Échec appel API Gupshup :', err);
      return { ok: false };
    }
  }
